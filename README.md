...
├─ .zcode/agents/aris-agent.md          # 单一通用审稿代理（见下）
├─ README.md                             # 本文件
└─ .git / .gitattributes
```
...
- `.zcode/agents/aris-agent.md` — 单一通用审稿代理
...
### 3.2 单一通用审稿代理 `aris-agent`

` .zcode/agents/aris-agent.md` 为极简通用代理：

```yaml
---
name: aris-agent
description: "General-purpose reviewer. Delegates to task prompt for role and checklist. Reads files at given paths."
...
- 10 个会调外部审稿的 skill（`research-review`、`proof-checker`、`experiment-audit`、`paper-claim-audit`、`citation-audit`、`kill-argument`、`rebuttal`、`idea-creator`、`auto-review-loop`、`research-refine`）在硬拷贝后会**幂等追加**分支：

  ```md
  <!-- ZCODE:ARIS-AGENT -->
  **If `Task(agent_type: aris-agent)` is available (ZCode default):**
    Use `Task(agent_type: aris-agent, prompt: "<same prompt that would go to codex>")`.
    No model or reasoning parameters needed per call.
    Hard `mcp__codex__codex` call remains as fallback when `aris-agent` is unavailable.
  ```

  优先走 `aris-agent`，不可用时回退到 `mcp__codex__codex`。vendor 文件永不被改。
...
| Agents（`aris-agent`） | ✅ 已完成 | 单一通用代理，prompt 决定角色 |
| AGENTS.md 托管块 | ✅ 已完成 | compare-and-swap，仅维护托管块 |
| Skill 审稿分支补丁（10 个） | ✅ 已完成 | 追加 `aris-agent` 分支，`mcp__codex__codex` 保留回退 |
...
- 审稿链路当前默认走 `aris-agent`，如环境未提供该 agent 则回退到 `mcp__codex__codex`；如需走 `llm-chat` 通用网关或 `manual-review`，按对应 skill 的 `REVIEWER_BACKEND` 选项配置。
