...
├─ .zcode/agents/gpt-reviewer.md          # 单一通用审稿代理（见下，名字即家族声明）
├─ README.md                             # 本文件
└─ .git / .gitattributes
```
...
- `templates/` — vendor `templates/`（21 个文件）硬拷贝到论文根目录，skill 内裸 `templates/XXX` 引用本地即达，无需 `$ARIS_REPO` 回源；用户改过的文件永不覆盖
...
- `.zcode/agents/gpt-reviewer.md` — 单一通用审稿代理（名字即家族声明）
...
### 3.2 单一通用审稿代理 `gpt-reviewer`

` .zcode/agents/gpt-reviewer.md` 为极简通用代理：
...
- 名字即家族声明：`gpt-reviewer` 表明默认 GPT 家族，与执行者（Claude）异源，满足上游同源检查（`provenance.py model_family()` 按 `gpt` 关键字归入 openai 家族）。换家族请同步改名（如 `deepseek-reviewer`）。
- 工具为只读加检索：`Read/Glob/Grep` 读文件，`Bash/WebFetch/WebSearch` 跑只读命令与外部检索（如引用核对、幻灯片渲染）。`TodoWrite` 仅子 agent 内部规划，不碰主 agent 的 todo。`Edit/Write` 保持关闭——审稿人只判分不改稿。
...
- 29 个会调外部审稿的 skill（机器扫描 `mcp__codex__codex:` 调用块得出的全集，另加编排型 `experiment-audit`）在硬拷贝后会**幂等追加**分支：
  ...
  **If `Task(agent_type: gpt-reviewer)` is available (ZCode default):**
  ...
  优先走 `gpt-reviewer`，不可用时回退到 `mcp__codex__codex`。vendor 文件永不被改。旧版 `aris-agent` / `ZCODE:ARIS-AGENT / ZCODE:GPT-WORKER (旧版，自动迁移)` 标记在重装时自动原位迁移，不重复追加。
...
- **`.aris/tools`、`.aris/installed-skills.txt`、`~/.aris/repo`** — 保持 ARIS 原生路径，使 `SKILL.md` 内 Canonical Helper 解析链无需改动：
  ...
  模板位于论文根目录 `templates/` 硬拷贝（见 §2）；`$ARIS_REPO/templates/XXX` 仅作回退。`.zcode/skills/shared-references/` 下的共享引用在 SKILL 点名时按需加载，不会自动注入。
...
| Agents（`gpt-reviewer`） | ✅ 已完成 | 单一通用代理，名字即家族声明，prompt 决定角色 |
| AGENTS.md 托管块 | ✅ 已完成 | compare-and-swap，仅维护托管块 |
| Skill 审稿分支补丁（29 个） | ✅ 已完成 | 追加 `gpt-reviewer` 分支，`mcp__codex__codex` 保留回退 |
...
- 模板位于论文根目录 `templates/` 硬拷贝（见 §2）。`.zcode/skills/shared-references/` 下的共享引用在 SKILL 点名时按需加载；`$ARIS_REPO`（manifest `repo_root` + `~/.aris/repo`）仅作 helper 回退，不再是模板主路径。
- 审稿链路当前默认走 `gpt-reviewer`，如环境未提供该 agent 则回退到 `mcp__codex__codex`；如需走 `llm-chat` 通用网关或 `manual-review`，按对应 skill 的 `REVIEWER_BACKEND` 选项配置。
